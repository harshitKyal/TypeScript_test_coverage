...
function centreWithMostSales(arr: any, prop: string): any {
    ...
    console.log(`The centre where we took the most sales is :`, max.centre);
    return max.centre;
...
function centreWithHighestPSM(arr: any[]): any {
    ...
    console.log('The store with the highest sales PSM is ' + result.centre)
    return result.centre;
}


// 1.
test('Total Sales in Chadstone', () => {
    expect(findTotalSalesInCentre(sales_data, "Chadstone")).toBe(6642);
})

// 2.
test('Top Sales centre', () => {
    expect(centreWithMostSales(sales_data, "totalSales")).toBe('Chadstone');
})

// 3.
test('Total Sales in Victoria', () => {
    expect(totalSalesOfState(sales_data, "VIC")).toBe(33599);
})

// 4.
test('Centre WIth Highest PSM', () => {
    expect(centreWithHighestPSM(sales_data)).toBe('Corio Central');
})